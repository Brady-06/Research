...
def run_finetune(
    ...
    overrides = {
        ...
        "imgsz": 640, "batch": 128, "device": device, "workers": 8,
...
def evaluate(weights: Path, data: Path, output_dir: Path, device: str) -> dict:
    ...
    metrics = model.val(
        data=str(Path(data).resolve()), imgsz=640, batch=128, device=device,
        workers=8, plots=False, verbose=False, seed=42, deterministic=True,
